...
namespace DoAn_LapTrinhWeb.Areas.Admin.Controllers
{
    public class AuthController : BaseController
    {
        ...
        public ActionResult Index(string search, int? size, int? page)
        {
            ...
            ViewBag.CurrentUserRole = User.Identity.GetRole(); // Thêm dòng này
        ...
        public JsonResult Disable(int id)
        {
            ...
            try
            {
                ...
                {
                    return Json(new { success = false, message = "Không thể vô hiệu hóa tài khoản của chính bạn" }, JsonRequestBehavior.AllowGet);
                }
                if (role != 0)
                {
                    return Json(new { success = false, message = "Chỉ Quản trị viên có quyền vô hiệu hóa tài khoản" }, JsonRequestBehavior.AllowGet);
                }
                account.status = "0";
                db.Configuration.ValidateOnSaveEnabled = false;
                db.Entry(account).State = EntityState.Modified;
                db.SaveChanges();
                return Json(new { success = true, message = "Vô hiệu hóa tài khoản thành công" }, JsonRequestBehavior.AllowGet);
            }
            catch
            {
